...
//Set duration time relatively larger than the original to obtain meaningful results.
...
void main()
{
	//set matrix of service time
	int service_time[] = { 0,0 };
	int staffs = 2;	// number of staffs
	...
	while (clock < duration) {
		...
		// Provide a service to customers whenever staff1 or staff2 are available.
		for (int i = 0; i < staffs; i++) {
			// Check if the customer who is receiving the service is finished.
			if (service_time[i] > 0)	// the customer is receiving service from ith server
				service_time[i]--;

			// no customer is receiving service. 
			// So, take out a customer from the queue and start the service.
			else {
				service_time[i] = remove_customer();
				// next customer will start to get a service from the staffs.
